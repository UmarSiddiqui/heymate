...
class WindowPositionManager {
    ...
    /// CGPreflight can stay false until the next launch even after the user
    /// clicks Allow. Remember a successful `CGRequestScreenCaptureAccess`.
    static func shouldPersistScreenRecordingGrant(
        preflightGranted: Bool,
        systemPromptReturnedGranted: Bool
    ) -> Bool {
        preflightGranted || systemPromptReturnedGranted
    }

    /// Setup only needs Accessibility, Screen Recording, and Microphone.
    /// The ScreenCaptureKit picker is requested automatically after Screen
    /// Recording is granted — it is not a fourth gate the user has to tap.
    ...
    static func persistScreenRecordingGrantIfNeeded(
        preflightGranted: Bool,
        systemPromptReturnedGranted: Bool
    ) {
        guard shouldPersistScreenRecordingGrant(
            preflightGranted: preflightGranted,
            systemPromptReturnedGranted: systemPromptReturnedGranted
        ) else { return }
        UserDefaults.standard.set(true, forKey: hasPreviouslyConfirmedScreenRecordingPermissionUserDefaultsKey)
    }
    ...
    static func requestScreenRecordingPermission() -> PermissionRequestPresentationDestination {
        ...
        case .systemPrompt:
            ...
            let systemPromptReturnedGranted = CGRequestScreenCaptureAccess()
            persistScreenRecordingGrantIfNeeded(
    ...
    static func privacyDropPlan(
        ...
        if isAlreadyInApplications {
            return .useExistingApplicationsCopy
        }
        if canCopyToApplications {
            return .copyToApplications
        }
        return .copyToDesktop
    }

    enum PrivacyBundleDropPlan: Equatable {
    ...
    static func prepareAppBundleForPrivacyDrop() -> URL {
        ...
        let plan = privacyDropPlan(
            isAlreadyInApplications: runningURL == applicationsTarget,
        ...
        case .copyToApplications:
            if copyAppBundle(from: runningURL, to: applicationsTarget) {
                return applicationsTarget
            }
            if copyAppBundle(from: runningURL, to: desktopTarget) {
    ...
    static func revealPreparedAppInFinder() {
        let url = prepareAppBundleForPrivacyDrop()
