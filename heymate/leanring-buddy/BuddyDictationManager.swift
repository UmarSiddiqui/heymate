...
enum BuddyPushToTalkShortcut {
        enum ShortcutOption: String, Hashable {
        case shiftFunction
        case controlOption
        case controlCommand
        case shiftControl
        ...
        static let allOptions: [ShortcutOption] = [
            .controlOption,
            .controlCommand,
            .shiftFunction,
            .shiftControl,
    ...
    private static let shortcutOptionUserDefaultsKey = "talkShortcutOption"

    /// The active push-to-talk shortcut. Persisted to UserDefaults so the
    /// choice survives relaunches. Defaults to ctrl+option (the classic
    /// Clicky-compatible default). Changing it takes effect immediately —
    /// the CGEvent tap consults this value on every event.
    static var currentShortcutOption: ShortcutOption {
        get {
            guard let rawValue = UserDefaults.standard.string(forKey: shortcutOptionUserDefaultsKey),
        ...
        set {
            UserDefaults.standard.set(newValue.rawValue, forKey: shortcutOptionUserDefaultsKey)
        }
    }

    private static let dictateOptionUserDefaultsKey = "dictateShortcutOption"

    /// The active contextual-dictation shortcut. Persisted; defaults to shift+fn.
    /// Must differ from the Talk shortcut — if both match, whichever tap started
    /// first wins (documented limitation).
    static var currentDictateOption: ShortcutOption {
        get {
            guard let rawValue = UserDefaults.standard.string(forKey: dictateOptionUserDefaultsKey),
                  let option = ShortcutOption(rawValue: rawValue) else { return .shiftFunction }
            return option
        }
        set { UserDefaults.standard.set(newValue.rawValue, forKey: dictateOptionUserDefaultsKey) }
    }

    private static let spatialOptionUserDefaultsKey = "spatialSelectShortcutOption"

    /// The active spatial-selection shortcut. Held down while the user drags a
    /// freehand region on screen; that region becomes priority context.
    static var currentSpatialOption: ShortcutOption {
        get {
            guard let rawValue = UserDefaults.standard.string(forKey: spatialOptionUserDefaultsKey),
                  let option = ShortcutOption(rawValue: rawValue) else { return .shiftControl }
            return option
        }
        set { UserDefaults.standard.set(newValue.rawValue, forKey: spatialOptionUserDefaultsKey) }
    }

    private static let chatOptionUserDefaultsKey = "chatShortcutOption"

    /// Summons the compact notch chat. Press toggles it open/closed; the
    /// keys do not need to be held. Defaults to ctrl+command.
    static var currentChatOption: ShortcutOption {
        get {
            guard let rawValue = UserDefaults.standard.string(forKey: chatOptionUserDefaultsKey),
                  let option = ShortcutOption(rawValue: rawValue) else { return .controlCommand }
            return option
        }
        set { UserDefaults.standard.set(newValue.rawValue, forKey: chatOptionUserDefaultsKey) }
    }

    static var pushToTalkDisplayText = currentShortcutOption.displayText
    static var pushToTalkTooltipText = "push to talk (\(currentShortcutOption.displayText))"

    private static let pushToTalkKeyCode: UInt16 = 49 // Space
...
final class BuddyDictationManager: NSObject, ObservableObject {
    ...
    func useTranscriptionProvider(_ provider: any BuddyTranscriptionProvider) {
    ...
    /// A nil resolution means "system default", which is what the engine
    /// already uses, so nothing is set. Failures are logged and ignored:
    /// capturing from the default microphone is much better than refusing to
    /// capture at all because a preferred device was unplugged.
    private func applySelectedAudioInputDevice(to inputNode: AVAudioInputNode) {
        guard let selectedAudioDeviceID = AudioInputDeviceCatalog.resolvedAudioDeviceID() else { return }

        var mutableAudioDeviceID = selectedAudioDeviceID
