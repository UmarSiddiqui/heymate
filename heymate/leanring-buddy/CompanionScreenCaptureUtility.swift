...
enum CompanionScreenCaptureUtility {
    ...
    /// instead of every display. A single tight image gives the vision model a
    /// sharper, cheaper frame to reason about — no desktop clutter and no
    /// second-monitor noise. Ported from danpeg/clicky's tutor mode.
    ///
    /// Falls back to `captureAllScreensAsJPEG()` when there is no matching
    /// window (e.g. Finder desktop, or our own app is frontmost because the
    /// desktop window is open), so opting in can never leave Talk blind.
    static func captureFocusedWindowAsJPEG() async throws -> [CompanionScreenCapture] {
        ...
        // The frontmost app's frontmost on-screen window. Our own windows are
        // skipped for the same reason as the full-screen capture: the AI
        // should see the user's content, never our overlays or panels.
        let focusedWindow = content.windows.first { window in
            ...
            // Tiny utility windows (floating toolbars, palettes) are not the
        ...
        // Convert the window's frame from Core Graphics global coordinates
        // (top-left origin of the primary display) to AppKit global
        // coordinates (bottom-left origin of the primary display), so
        // displayFrame matches NSEvent.mouseLocation and the coordinate math
        // in ScreenCoordinateMath. Claude's POINT coordinates come back
        // relative to this image, which contains only the window — so the
        // capture's size and origin must be the window's, not the display's.
        ...
        return [CompanionScreenCapture(
            imageData: jpegData,
            label: windowLabel,
            // This is the only capture, so it must also be the pointing
            // target: the cursor-screen fallback in applyPointingParseResult
            // would otherwise find nothing when the pointer sits outside the
            // window (e.g. resting on a second monitor).
            isCursorScreen: true,
